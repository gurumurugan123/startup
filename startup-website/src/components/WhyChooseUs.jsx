      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {whyChooseUs.map((item, index) => {
          ...
          return (
            <div
              key={item}
              className="p-6 rounded-2xl bg-white shadow-soft transform transition duration-500 hover:-translate-y-2 hover:shadow-lg hover:scale-105 opacity-0 animate-fadeIn"
              style={{ animationDelay: `${index * 0.15}s` }} // staggered animation
            >
              <div className="flex items-center gap-2 mb-2">
                <Icon className="w-6 h-6 text-brandPurple animate-bounce-slow" />
                <span className="text-brandPurple font-semibold">{item}</span>
              </div>
              <p className="text-gray-600 text-sm">We focus on results, quality, and support.</p>
            </div>
          );
        })}
      </div>
